...
func TestFeedReplyRetryInstructionKeepsFeedRepliesSubtle(t *testing.T) {
	...
	for _, want := range []string{"原始指令", "太像角色表演", "补回惠惠式反应", "不要退成中立路人", "被帖子刺激到的第一反应", "不要每次都用动作描写开场", "普通短评默认1-2句", "专席、报委托、委托栏、转职路线、传送阵、领成就、卷轴"} {
...
func TestBuildFeedReplyInstructionUsesTavernReplyStyle(t *testing.T) {
	...
	for _, want := range []string{"符合上下文的评论", "普通回复一样的酒馆人设", "自然接话", "不能退成中立路人", "惠惠式反应", "被帖子刺激到的第一反应", "不要每条都用动作描写开场", "测试帖子", "正文摘要"} {
		if !strings.Contains(got, want) {
