...
func buildImageReplyText(linkID, rootID, commentID, userID int, originalText string, dryRun bool) string {
	...
	return normalizeImageReplyText(ai.GetAiReplyWithoutCharacterCard(contents, instruction, topics, tags, zap.Int("comment_id", commentID), zap.Int("link_id", linkID), zap.Int("user_id", userID), zap.String("question", originalText)))
}
