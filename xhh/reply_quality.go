...
func aiReplyRetryInstruction(questionText, issue string) string {
	...
	builder.WriteString("。请重新生成。要求：像当前配置的人设本人在小黑盒评论区自然接话；先回应对方说的话；用态度、情绪和判断体现人设，不要靠反复自称名字、种族、招牌技能或口头禅证明人设；动作描写只能少量点到，不要一条回复里连续舞台表演；不要客服腔；不要输出 SKIP；默认1-3句。")
	builder.WriteString("\nNatural rewrite note: answer the user's actual words first; be willing to play along with harmless jokes, teasing, nicknames, and non-sexual roleplay. Do not stack persona terms such as 红魔族、爆裂魔法、本大魔法师、委托、召唤、咒文 in one reply. Do not generate explicit sexual content, pornographic descriptions, or erotic roleplay; if the user pushes that way, deflect briefly in character and offer a non-sexual angle.")
